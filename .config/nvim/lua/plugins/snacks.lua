local logo_path = vim.fn.stdpath("config") .. "/logo.png" -- 建议用 png，jpeg 也可以试
local img_cmd = "chafa " .. logo_path .. " --format symbols --symbols vhalf --stretch; sleep .1"
return {

  {
    ...
    opts = {
      ...
      dashboard = {
        ...
        -- },
        sections = {
          -- [核心修改] 使用 terminal section 来运行 imgcat
          {
            section = "terminal",
            cmd = img_cmd,
            -- 调整高度：终端模式下，图片高度由这个 height 决定
            height = 30,
            -- 这里的 width 只是占位宽度，不影响图片实际拉伸，但影响居中
            width = 60,
            padding = 1,
            indent = 6, -- 稍微向右缩进一点以居中
          },
          -- 下面是其他的组件
          { section = "keys", gap = 1, padding = 1 },
          { section = "startup" },
...
⠀⠈⠛⠶⣤⣤⣀⣀⣉⣉⣉⣉⣉⣀⣀⣀⣠⣤⣤⠴⠖⠋⠁⠀⠈⠙⠲⠶⣤⣤⣄⣀⣀⣀⣉⣉⣉⣉⣉⣁⣀⣤⣤⠴⠞⠋⠀⠀
      ...
      image = {
        enabled = true,
        doc = { inline = false, float = false, max_width = 128, max_height = 128 },
